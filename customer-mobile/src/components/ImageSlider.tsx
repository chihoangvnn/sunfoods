...
interface ImageSliderProps {
  images: string[]; // Can be image or video URLs
  className?: string;
  autoplay?: boolean;
...
export const ImageSlider = memo(function ImageSlider({ 
  ...
  useEffect(() => {
    if (!autoplay || images.length <= 1 || isInteracting) {
  ...
  }, [currentIndex, autoplay, autoplayDelay, images.length, isInteracting]);
  ...
  return (
    <div className={`relative w-full overflow-hidden bg-gray-100 ${className}`}>
      {/* Slider Container with responsive aspect ratios - 16:9 mobile, 21:9 desktop */}
      <div
        ref={sliderRef}
        className={`relative w-full aspect-[16/9] lg:aspect-[21/9] select-none touch-pan-y ${
      ...
      >
        ...
        >
          {images.map((mediaUrl, index) => (
            <div key={index} className={`w-full h-full flex-shrink-0 ${isVideoUrl(mediaUrl) ? 'bg-black' : ''}`}>
              {isVideoUrl(mediaUrl) ? (
                <video
                  src={mediaUrl}
                  autoPlay
