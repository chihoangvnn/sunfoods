...
const updateShopSettingsSchema = z.object({
  ...
  // Ratings (coerce from string to number to handle form inputs, or accept empty string)
  rating: z.string().optional().or(z.coerce.number().min(0).max(5)),
  ...
  shopLongitude: z.string().optional().or(z.literal('')),
  localRadiusKm: z.coerce.number().positive().optional()
