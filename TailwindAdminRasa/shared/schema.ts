...
export const affiliateClicks = pgTable("affiliate_clicks", {
  ...
  conversionValue: numeric({ precision: 15, scale: 2 }),
...
export const affiliateLandingPages = pgTable("affiliate_landing_pages", {
  ...
  totalRevenue: numeric({ precision: 15, scale: 2 }).default(0.00),
  conversionRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  isActive: boolean("is_active").default(true).notNull(),
...
export const affiliateProductAssignments = pgTable("affiliate_product_assignments", {
  ...
  maxCommissionPerOrder: numeric({ precision: 15, scale: 2 }),
  totalSales: integer("total_sales").default(0).notNull(),
  totalCommission: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  assignedBy: varchar("assigned_by"),
  assignedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
...
export const affiliateProductRequests = pgTable("affiliate_product_requests", {
  ...
  suggestedPrice: numeric({ precision: 15, scale: 2 }),
  categoryId: varchar("category_id"),
  status: text().default('pending').notNull(),
  requestReason: text("request_reason"),
  adminNotes: text("admin_notes"),
  approvedProductId: varchar("approved_product_id"),
  approvedCommissionRate: numeric({ precision: 5, scale: 2 }),
...
export const apiConfigurations = pgTable("api_configurations", {
  ...
  avgResponseTime: numeric({ precision: 10, scale: 3 }).default(0),
  lastAccessed: timestamp({ mode: 'string' }),
  lastToggled: timestamp({ mode: 'string' }),
  lastError: timestamp({ mode: 'string' }),
  tags: jsonb().default([]),
...
export const bookAnalytics = pgTable("book_analytics", {
  id: varchar().default(sql`gen_random_uuid()`).primaryKey(),
  bookIsbn: varchar({ length: 17 }).notNull(),
  ...
  lowestPrice: numeric({ precision: 10, scale: 2 }),
  highestPrice: numeric({ precision: 10, scale: 2 }),
  averagePrice: numeric({ precision: 10, scale: 2 }),
  totalRatings: integer("total_ratings").default(0),
  averageRating: numeric({ precision: 3, scale: 2 }),
  lastViewedAt: timestamp({ mode: 'string' }),
  lastPurchasedAt: timestamp({ mode: 'string' }),
  analyticsDate: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const bookCustomers = pgTable("book_customers", {
  ...
  totalSpent: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  totalBooks: integer("total_books").default(0).notNull(),
  avgOrderValue: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  lastPurchase: timestamp({ mode: 'string' }),
...
export const bookOrderItems = pgTable("book_order_items", {
  ...
  sellerPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  marketPrice: numeric({ precision: 15, scale: 2 }),
  sourceCost: numeric({ precision: 15, scale: 2 }),
});
...
export const bookOrders = pgTable("book_orders", {
  ...
  vtpCreatedAt: timestamp({ mode: 'string' }),
  vtpUpdatedAt: timestamp({ mode: 'string' }),
  sellerId: varchar("seller_id"),
  bookSource: text("book_source").default('local_inventory').notNull(),
  isbn: text(),
  condition: text().default('new').notNull(),
  sellerCommission: numeric({ precision: 15, scale: 2 }).default(0),
...
export const bookPrices = pgTable("book_prices", {
  ...
  lastUpdatedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  sourceId: varchar("source_id"),
  currency: varchar({ length: 3 }).default('USD'),
  stock: integer().default(0),
  condition: varchar({ length: 20 }).default('new'),
  sellerName: varchar({ length: 100 }),
  sellerRating: numeric({ precision: 3, scale: 2 }),
  deliveryTime: varchar({ length: 50 }),
  isCurrentPrice: boolean("is_current_price").default(true),
  priceHistory: jsonb("price_history"),
  lastChecked: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
}, (table) => [
...
export const bookSellerInventory = pgTable("book_seller_inventory", {
  ...
  basePrice: numeric({ precision: 15, scale: 2 }).notNull(),
  sellerPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  calculatedPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  assignedAt: timestamp({ mode: 'string' }).defaultNow(),
  assignmentType: text("assignment_type").default('auto_random').notNull(),
  totalSold: integer("total_sold").default(0).notNull(),
  totalRevenue: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  lastSale: timestamp({ mode: 'string' }),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
}, (table) => [
...
export const bookSellers = pgTable("book_sellers", {
  ...
  totalSales: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  totalOrders: integer("total_orders").default(0).notNull(),
  avgRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
...
export const books = pgTable("books", {
  ...
  averageRating: numeric({ precision: 3, scale: 2 }).default(0.0),
  ...
  lastPriceUpdate: timestamp({ mode: 'string' }),
  sellerId: varchar({ length: 50 }),
});
...
export const campaignParticipations = pgTable("campaign_participations", {
  ...
  status: text().default('pending').notNull(),
  verificationScheduledAt: timestamp({ mode: 'string' }),
  lastVerifiedAt: timestamp({ mode: 'string' }),
  rewardedAt: timestamp({ mode: 'string' }),
...
export const competitorProfiles = pgTable("competitor_profiles", {
  ...
  marketShare: numeric({ precision: 5, scale: 4 }).default(0.0500).notNull(),
  avgRating: numeric({ precision: 3, scale: 2 }).default(4.00).notNull(),
  totalReviews: integer("total_reviews").default(500).notNull(),
  inventorySize: integer("inventory_size").default(5000).notNull(),
  avgDiscountRate: numeric({ precision: 5, scale: 4 }).default(0.1000).notNull(),
  priceChangeFrequency: text("price_change_frequency").default('weekly').notNull(),
  priceFlexibility: numeric({ precision: 5, scale: 2 }).default(15.00).notNull(),
  ...
  seasonalAggressiveness: numeric({ precision: 3, scale: 2 }).default(1.50).notNull(),
  ...
  isActive: boolean("is_active").default(true).notNull(),
  simulationWeight: numeric({ precision: 3, scale: 2 }).default(1.00).notNull(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const consignmentRequests = pgTable("consignment_requests", {
  ...
  proposedPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  discountPercent: numeric({ precision: 5, scale: 2 }).default(0),
...
export const contentAssets = pgTable("content_assets", {
  ...
  cloudinaryPublicId: varchar({ length: 255 }).notNull(),
  ...
  resourceType: varchar({ length: 20 }).notNull(),
}, (table) => [
...
export const contentFaqAssignments = pgTable("content_faq_assignments", {
  ...
  inheritedAt: timestamp({ mode: 'string' }),
}, (table) => [
...
export const contentQueue = pgTable("content_queue", {
  ...
  lastScheduledAt: timestamp({ mode: 'string' }),
  errorMessage: text("error_message"),
...
export const conversationSessions = pgTable("conversation_sessions", {
  ...
  avgResponseTime: numeric({ precision: 8, scale: 2 }),
  status: text().default('active').notNull(),
  ...
  lastActiveAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
}, (table) => [
...
export const customerReviews = pgTable("customer_reviews", {
  ...
  reviewLanguage: varchar({ length: 10 }).default('vi').notNull(),
  regionDialect: varchar({ length: 20 }).default('miền-bắc'),
  isAutoGenerated: boolean("is_auto_generated").default(false).notNull(),
  sentimentScore: numeric({ precision: 3, scale: 2 }).default(0),
...
export const customerVouchers = pgTable("customer_vouchers", {
  ...
  claimedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
  ...
  orderId: varchar("order_id"),
  discountApplied: numeric({ precision: 10, scale: 2 }),
  createdAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
});
...
export const customers = pgTable("customers", {
  ...
  avatar: text(),
  status: text().default('active').notNull(),
  joinDate: timestamp({ mode: 'string' }).defaultNow(),
  totalDebt: numeric({ precision: 15, scale: 2 }).default(0),
  creditLimit: numeric({ precision: 15, scale: 2 }).default(0),
  membershipTier: text("membership_tier").default('member'),
  totalSpent: numeric({ precision: 15, scale: 2 }).default(0),
  pointsBalance: integer("points_balance").default(0),
  pointsEarned: integer("points_earned").default(0),
  lastTierUpdate: timestamp({ mode: 'string' }).defaultNow(),
  ...
  commissionRate: numeric({ precision: 5, scale: 2 }).default(5.00),
  ...
  distanceFromShop: numeric({ precision: 10, scale: 3 }),
  geocodingStatus: text("geocoding_status").default('not_geocoded'),
  lastGeocodedAt: timestamp({ mode: 'string' }),
  address2: text(),
}, (table) => [
...
export const depositTransactions = pgTable("deposit_transactions", {
  ...
  amount: numeric({ precision: 15, scale: 2 }).notNull(),
  balanceBefore: numeric({ precision: 15, scale: 2 }),
  balanceAfter: numeric({ precision: 15, scale: 2 }),
...
export const discountCodeUsages = pgTable("discount_code_usages", {
  ...
  orderId: varchar("order_id"),
  customerId: varchar("customer_id"),
  usageAmount: numeric({ precision: 10, scale: 2 }).notNull(),
  discountApplied: numeric({ precision: 10, scale: 2 }).notNull(),
  channel: varchar().default('online'),
  usedAt: timestamp({ mode: 'string' }).defaultNow(),
  metadata: jsonb(),
});
...
export const discountCodes = pgTable("discount_codes", {
  ...
  maxDiscountAmount: numeric({ precision: 10, scale: 2 }),
  tierRules: jsonb("tier_rules"),
  maxUsage: integer("max_usage"),
  maxUsagePerCustomer: integer("max_usage_per_customer").default(1),
  minOrderAmount: numeric({ precision: 10, scale: 2 }).default(0),
...
export const driverReports = pgTable("driver_reports", {
  ...
  status: text().default('pending').notNull(),
  resolution: text(),
  reportedAt: timestamp({ mode: 'string' }).defaultNow(),
  reviewedAt: timestamp({ mode: 'string' }),
...
export const facebookApps = pgTable("facebook_apps", {
  ...
  description: text(),
  lastWebhookEvent: timestamp({ mode: 'string' }),
...
export const faqGenerationResults = pgTable("faq_generation_results", {
  ...
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
  confidenceScore: numeric({ precision: 5, scale: 3 }).default(0.85),
  reviewedBy: varchar("reviewed_by"),
  reviewedAt: timestamp({ mode: 'string' }),
...
export const generalCategoryAnalytics = pgTable("general_category_analytics", {
  id: varchar().default(sql`gen_random_uuid()`).primaryKey(),
  categoryId: varchar("category_id").notNull(),
  analyticsDate: timestamp({ mode: 'string' }).defaultNow().notNull(),
  pageViews: integer("page_views").default(0),
  uniqueVisitors: integer("unique_visitors").default(0),
  bounceRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  avgTimeOnPage: integer("avg_time_on_page").default(0),
  totalProducts: integer("total_products").default(0),
  activeProducts: integer("active_products").default(0),
  totalOrders: integer("total_orders").default(0),
  totalRevenue: numeric({ precision: 12, scale: 2 }).default(0.00),
  clickThroughRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  conversionRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const generalCategoryPriceRules = pgTable("general_category_price_rules", {
  ...
  markupPercentage: numeric({ precision: 5, scale: 2 }),
  conditions: jsonb().default({}),
  priority: integer().default(0),
  isActive: boolean("is_active").default(true),
...
export const giftCampaigns = pgTable("gift_campaigns", {
  ...
  activeFrom: timestamp({ mode: 'string' }).defaultNow(),
  activeUntil: timestamp({ mode: 'string' }),
  totalSold: integer("total_sold").default(0).notNull(),
  totalValue: numeric({ precision: 12, scale: 2 }).default(0).notNull(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdBy: varchar("created_by"),
});
...
export const giftRedemptions = pgTable("gift_redemptions", {
  ...
  orderId: varchar("order_id"),
  redeemedAmount: numeric({ precision: 10, scale: 2 }).notNull(),
  ...
  redeemedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const giftVouchers = pgTable("gift_vouchers", {
  id: varchar().default(sql`gen_random_uuid()`).primaryKey(),
  voucherCode: varchar({ length: 20 }).notNull(),
  campaignId: varchar("campaign_id").notNull(),
  purchaseOrderId: varchar("purchase_order_id"),
  purchasedBy: varchar("purchased_by"),
  purchaseAmount: numeric({ precision: 10, scale: 2 }).notNull(),
  purchaseChannel: varchar("purchase_channel").notNull(),
  faceValue: numeric({ precision: 10, scale: 2 }).notNull(),
  ...
  firstUsedAt: timestamp({ mode: 'string' }),
  fullyRedeemedAt: timestamp({ mode: 'string' }),
  ...
  issuedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
}, (table) => [
...
export const intentAnalytics = pgTable("intent_analytics", {
  ...
  successRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  avgConfidence: numeric({ precision: 4, scale: 3 }),
  avgResponseTime: numeric({ precision: 8, scale: 2 }),
...
export const ipPoolSessions = pgTable("ip_pool_sessions", {
  ...
  sessionStart: timestamp({ mode: 'string' }).defaultNow().notNull(),
  sessionEnd: timestamp({ mode: 'string' }),
...
export const ipPools = pgTable("ip_pools", {
  ...
  currentIp: varchar({ length: 100 }),
  config: jsonb().default({}),
  healthScore: integer("health_score").default(100),
  totalRotations: integer("total_rotations").default(0),
  lastRotatedAt: timestamp({ mode: 'string' }),
  isEnabled: boolean("is_enabled").default(true),
  priority: integer().default(0),
  costPerMonth: numeric({ precision: 10, scale: 2 }),
  notes: text(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const ipRotationLogs = pgTable("ip_rotation_logs", {
  ...
  ipPoolId: integer("ip_pool_id").notNull(),
  oldIp: varchar({ length: 100 }),
  newIp: varchar({ length: 100 }),
  rotationReason: varchar({ length: 255 }),
  rotationMethod: varchar({ length: 50 }),
...
export const marketTrends = pgTable("market_trends", {
  ...
  trendScore: numeric({ precision: 5, scale: 2 }).default(50.00).notNull(),
  trendDirection: text("trend_direction").default('stable').notNull(),
  momentumScore: numeric({ precision: 5, scale: 2 }).default(0).notNull(),
  searchVolume: integer("search_volume").default(0).notNull(),
  searchVolumeChange: numeric({ precision: 7, scale: 2 }).default(0).notNull(),
  orderVolumeWeekly: integer("order_volume_weekly").default(0).notNull(),
  averagePrice: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  priceVolatility: numeric({ precision: 5, scale: 2 }).default(0).notNull(),
  competitorCount: integer("competitor_count").default(0).notNull(),
  marketSaturation: numeric({ precision: 5, scale: 4 }).default(0.5000).notNull(),
  entryBarrier: text("entry_barrier").default('medium').notNull(),
  seasonalityIndex: numeric({ precision: 5, scale: 2 }).default(1.00).notNull(),
  peakSeasons: jsonb("peak_seasons").default([]),
  vietnamesePopularity: numeric({ precision: 5, scale: 2 }).default(50.00).notNull(),
  ...
  confidenceScore: numeric({ precision: 3, scale: 2 }).default(0.50).notNull(),
  ...
  lastCalculated: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const orders = pgTable("orders", {
  ...
  shippingFee: numeric({ precision: 10, scale: 2 }).default(5.00),
  ...
  paymentStatus: varchar({ length: 20 }).default('pending'),
  ...
  affiliateId: varchar("affiliate_id"),
  affiliateCommission: numeric({ precision: 15, scale: 2 }).default(0.00),
...
export const performanceMetrics = pgTable("performance_metrics", {
  ...
  totalRevenue: numeric({ precision: 15, scale: 2 }).default(0),
  averageOrderValue: numeric({ precision: 10, scale: 2 }).default(0),
  averageResponseTimeHours: numeric({ precision: 5, scale: 2 }).default(0),
  fulfillmentAccuracyPercent: numeric({ precision: 5, scale: 2 }).default(0),
  customerSatisfactionScore: numeric({ precision: 3, scale: 2 }).default(0),
  repeatCustomerRate: numeric({ precision: 5, scale: 2 }).default(0),
  recommendationRate: numeric({ precision: 5, scale: 2 }).default(0),
  culturalSensitivityScore: numeric({ precision: 3, scale: 2 }).default(0),
  efficiencyScore: numeric({ precision: 3, scale: 2 }).default(0),
  qualityScore: numeric({ precision: 3, scale: 2 }).default(0),
  overallPerformanceScore: numeric({ precision: 3, scale: 2 }).default(0),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const pricingStrategies = pgTable("pricing_strategies", {
  ...
  baseProfitMargin: numeric({ precision: 5, scale: 4 }).default(0.2000).notNull(),
  minProfitMargin: numeric({ precision: 5, scale: 4 }).default(0.0500).notNull(),
  maxProfitMargin: numeric({ precision: 5, scale: 4 }).default(0.5000).notNull(),
  ...
  competitiveMargin: numeric({ precision: 5, scale: 4 }).default(0.0500).notNull(),
  demandElasticity: numeric({ precision: 5, scale: 4 }).default(0.5000).notNull(),
  stockLevelPricing: boolean("stock_level_pricing").default(true).notNull(),
  lowStockPremium: numeric({ precision: 5, scale: 4 }).default(0.1000).notNull(),
  overStockDiscount: numeric({ precision: 5, scale: 4 }).default(0.1500).notNull(),
  ...
  maxPriceChangePerDay: numeric({ precision: 5, scale: 4 }).default(0.1000).notNull(),
  avgPriceChangePercent: numeric({ precision: 7, scale: 4 }).default(0).notNull(),
  profitabilityScore: numeric({ precision: 5, scale: 2 }).default(50.00).notNull(),
  competitivenessScore: numeric({ precision: 5, scale: 2 }).default(50.00).notNull(),
  salesVelocityImpact: numeric({ precision: 7, scale: 4 }).default(0).notNull(),
  isActive: boolean("is_active").default(true).notNull(),
  lastAdjusted: timestamp({ mode: 'string' }),
  nextAdjustment: timestamp({ mode: 'string' }),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const productLandingPages = pgTable("product_landing_pages", {
  ...
  customPrice: numeric({ precision: 15, scale: 2 }),
  originalPrice: numeric({ precision: 15, scale: 2 }),
  ...
  conversionRate: numeric({ precision: 5, scale: 2 }).default(0.00).notNull(),
...
export const products = pgTable("products", {
  ...
  minQuantity: numeric({ precision: 10, scale: 3 }).default(1.000),
  quantityStep: numeric({ precision: 10, scale: 3 }).default(1.000),
  ...
  localPrice: numeric({ precision: 15, scale: 2 }),
  standardPrice: numeric({ precision: 15, scale: 2 }),
}, (table) => [
...
export const projectTemplates = pgTable("project_templates", {
  ...
  appliedAt: timestamp({ mode: 'string' }).defaultNow(),
  isActive: boolean("is_active").default(true).notNull(),
...
export const queueAutofillSettings = pgTable("queue_autofill_settings", {
  ...
  similarityThreshold: numeric({ precision: 3, scale: 2 }).default(0.8),
...
export const salesAutomationConfigs = pgTable("sales_automation_configs", {
  ...
  totalAutomatedRevenue: numeric({ precision: 15, scale: 2 }).default(0),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const satisfactionSurveys = pgTable("satisfaction_surveys", {
  ...
  customerId: varchar("customer_id"),
  orderId: varchar("order_id"),
  surveyType: varchar({ length: 50 }).default('post_purchase').notNull(),
  ...
  surveyLanguage: varchar({ length: 10 }).default('vi'),
...
export const seasonalRules = pgTable("seasonal_rules", {
  ...
  priceMultiplier: numeric({ precision: 5, scale: 4 }).default(1.0000).notNull(),
  maxPriceIncrease: numeric({ precision: 5, scale: 4 }).default(0.2000).notNull(),
  maxPriceDecrease: numeric({ precision: 5, scale: 4 }).default(0.3000).notNull(),
  dynamicPricingEnabled: boolean("dynamic_pricing_enabled").default(false).notNull(),
  inventoryMultiplier: numeric({ precision: 5, scale: 4 }).default(1.0000).notNull(),
  ...
  lastApplied: timestamp({ mode: 'string' }),
  timesApplied: integer("times_applied").default(0).notNull(),
  avgImpactScore: numeric({ precision: 5, scale: 2 }).default(0).notNull(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const sellerPaymentConfigs = pgTable("seller_payment_configs", {
  ...
  sellerId: varchar("seller_id"),
  commissionRate: numeric({ precision: 5, scale: 2 }).default(15.00).notNull(),
  minCommission: numeric({ precision: 15, scale: 2 }).default(0.00),
...
export const sellerRatings = pgTable("seller_ratings", {
  id: varchar().default(sql`gen_random_uuid()`).primaryKey(),
  sellerId: varchar("seller_id").notNull(),
  overallRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  totalReviews: integer("total_reviews").default(0).notNull(),
  totalRatings: integer("total_ratings").default(0).notNull(),
  deliverySpeedRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  bookConditionRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  customerServiceRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  pricingRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  culturalSensitivityRating: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  responseTimeHours: numeric({ precision: 5, scale: 2 }).default(0).notNull(),
  fulfillmentAccuracyPercent: numeric({ precision: 5, scale: 2 }).default(0).notNull(),
  lastUpdatedAt: timestamp({ mode: 'string' }).defaultNow(),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
});
...
export const shippingZones = pgTable("shipping_zones", {
  ...
  name: text().notNull(),
  description: text(),
  minDistanceKm: numeric({ precision: 10, scale: 2 }).default(0).notNull(),
  maxDistanceKm: numeric({ precision: 10, scale: 2 }).notNull(),
  shippingFee: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
...
export const shopSettings = pgTable("shop_settings", {
  ...
  shopLatitude: numeric({ precision: 10, scale: 8 }),
  shopLongitude: numeric({ precision: 11, scale: 8 }),
  localRadiusKm: numeric({ precision: 6, scale: 2 }).default(20.00),
});
...
export const shopeeBusinessAccounts = pgTable("shopee_business_accounts", {
  ...
  responseRate: numeric({ precision: 5, scale: 2 }).default(0.00),
  responseTime: integer("response_time").default(0),
  followerCount: integer("follower_count").default(0),
  productCount: integer("product_count").default(0),
  totalOrders: integer("total_orders").default(0),
  totalRevenue: numeric({ precision: 15, scale: 2 }).default(0),
  avgOrderValue: numeric({ precision: 15, scale: 2 }).default(0),
  lastOrderAt: timestamp({ mode: 'string' }),
...
export const shopeeShopOrders = pgTable("shopee_shop_orders", {
  ...
  currency: text().default('VND').notNull(),
  actualShippingFee: numeric({ precision: 15, scale: 2 }).default(0),
  goodsToReceive: numeric({ precision: 15, scale: 2 }).default(0),
  coinOffset: numeric({ precision: 15, scale: 2 }).default(0),
  escrowAmount: numeric({ precision: 15, scale: 2 }).default(0),
  items: jsonb().notNull(),
  shippingCarrier: text("shipping_carrier"),
  trackingNumber: text("tracking_number"),
  shipTime: timestamp({ mode: 'string' }),
  ...
  shopeeFee: numeric({ precision: 15, scale: 2 }).default(0),
  transactionFee: numeric({ precision: 15, scale: 2 }).default(0),
  commissionFee: numeric({ precision: 15, scale: 2 }).default(0),
  serviceFee: numeric({ precision: 15, scale: 2 }).default(0),
  tagIds: jsonb("tag_ids").default([]),
  notes: text(),
  createTime: timestamp({ mode: 'string' }).notNull(),
  updateTime: timestamp({ mode: 'string' }),
  payTime: timestamp({ mode: 'string' }),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow(),
}, (table) => [
...
export const shopeeShopProducts = pgTable("shopee_shop_products", {
  ...
  description: text(),
  originalPrice: numeric({ precision: 15, scale: 2 }),
  currentPrice: numeric({ precision: 15, scale: 2 }),
  stock: integer(),
...
export const socialAccounts = pgTable("social_accounts", {
  ...
  connected: boolean().default(false),
  lastPost: timestamp({ mode: 'string' }),
  ...
  performanceScore: numeric({ precision: 5, scale: 2 }).default(0),
  lastOptimization: timestamp({ mode: 'string' }),
...
export const tiktokShopOrders = pgTable("tiktok_shop_orders", {
  ...
  totalAmount: numeric({ precision: 15, scale: 2 }).notNull(),
  currency: text().default('VND').notNull(),
  taxAmount: numeric({ precision: 15, scale: 2 }).default(0),
  shippingAmount: numeric({ precision: 15, scale: 2 }).default(0),
  discountAmount: numeric({ precision: 15, scale: 2 }).default(0),
  items: jsonb().notNull(),
  ...
  tiktokFees: numeric({ precision: 15, scale: 2 }).default(0),
  notes: text(),
...
export const tiktokShopProducts = pgTable("tiktok_shop_products", {
  ...
  tiktokPrice: numeric({ precision: 15, scale: 2 }),
  ...
  conversionRate: numeric({ precision: 5, scale: 2 }).default(0),
  lastSyncAt: timestamp({ mode: 'string' }),
  syncStatus: text("sync_status").default('pending'),
  syncError: text("sync_error"),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
...
export const tiktokVideos = pgTable("tiktok_videos", {
  ...
  comments: integer().default(0),
  shares: integer().default(0),
  engagementRate: numeric({ precision: 5, scale: 2 }).default(0),
  shopProductsTagged: jsonb("shop_products_tagged").default([]),
  salesFromVideo: numeric({ precision: 15, scale: 2 }).default(0),
  clickthroughRate: numeric({ precision: 5, scale: 2 }).default(0),
...
export const trips = pgTable("trips", {
  ...
  actualArrivalTime: timestamp({ mode: 'string' }),
  ticketPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  currency: text().default('VND').notNull(),
  ...
  netProfit: numeric({ precision: 15, scale: 2 }).default(0),
  notes: text(),
...
export const vehicles = pgTable("vehicles", {
  ...
  cargoCapacity: numeric({ precision: 10, scale: 2 }),
  registrationNumber: text("registration_number"),
  registrationExpiry: timestamp({ mode: 'string' }),
...
export const vendorOrders = pgTable("vendor_orders", {
  ...
  codAmount: numeric({ precision: 15, scale: 2 }),
  vendorCost: numeric({ precision: 15, scale: 2 }),
  commissionAmount: numeric({ precision: 15, scale: 2 }),
  depositDeducted: boolean("deposit_deducted").default(false),
  status: varchar().default('pending'),
  processingAt: timestamp({ mode: 'string' }),
  shippedAt: timestamp({ mode: 'string' }),
  deliveredAt: timestamp({ mode: 'string' }),
...
export const vendorProducts = pgTable("vendor_products", {
  ...
  consignmentPrice: numeric({ precision: 15, scale: 2 }).notNull(),
  discountPercent: numeric({ precision: 5, scale: 2 }).default(0),
  consignmentDate: timestamp({ mode: 'string' }).defaultNow().notNull(),
  expiryDate: timestamp({ mode: 'string' }),
  status: varchar().default('active'),
  createdAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
  updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
  commissionPerUnit: numeric({ precision: 15, scale: 2 }).default(0.00),
}, (table) => [
...
export const vendors = pgTable("vendors", {
  ...
  depositBalance: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  depositTotal: numeric({ precision: 15, scale: 2 }).default(0).notNull(),
  minimumDeposit: numeric({ precision: 15, scale: 2 }).default(1000000).notNull(),
  paymentMethod: varchar("payment_method").default('deposit'),
  bankInfo: jsonb("bank_info").default({}),
  commissionRate: numeric({ precision: 5, scale: 2 }).default(0.30).notNull(),
  ...
  monthlyDebt: numeric({ precision: 15, scale: 2 }).default(0.00).notNull(),
}, (table) => [
...
export const vietnameseReviewTemplates = pgTable("vietnamese_review_templates", {
  id: varchar().default(sql`gen_random_uuid()`).primaryKey(),
  templateName: text("template_name").notNull(),
  templateCategory: varchar({ length: 20 }).notNull(),
  regionDialect: varchar({ length: 20 }).default('miền-bắc').notNull(),
  bookCategory: varchar({ length: 50 }),
  reviewTitleTemplate: text("review_title_template").notNull(),
  reviewContentTemplate: text("review_content_template").notNull(),
  sentimentScore: numeric({ precision: 3, scale: 2 }).default(0).notNull(),
  formalityLevel: varchar({ length: 20 }).default('casual'),
  ageGroup: varchar({ length: 20 }).default('adult'),
  usageCount: integer("usage_count").default(0),
  isActive: boolean("is_active").default(true),
  tags: text().default('{}'),
  createdAt: timestamp({ mode: 'string' }).defaultNow(),
...
export const workers = pgTable("workers", {
  ...
  lastPingAt: timestamp({ mode: 'string' }),
  lastJobAt: timestamp({ mode: 'string' }),
  ...
  successRate: numeric({ precision: 5, scale: 2 }).default(0.00).notNull(),
